;;; custom-workspace.el ---                                   -*- lexical-binding: t; -*-
...
;;;###autoload
(defalias '+workspace/restore-last-session #'dotfairy/quickload-session)
        ...
        (if (not (member name workspaces))
          ...
          (cond ((delq (selected-frame) (persp-frames-with-persp (get-frame-persp)))
                ...
                ((cdr workspaces)
                 ...
                 (unless (dotfairy-buffer-frame-predicate (window-buffer))
                   (switch-to-buffer (dotfairy-fallback-buffer))))
                (t
                 (+workspace-switch +workspaces-main t)
                 (unless (string= (car workspaces) +workspaces-main)
                   (+workspace-delete name))
                 (dotfairy/kill-all-buffers (dotfairy-buffer-list))))
  ...
  (when persp-mode
    (if (not (persp-frame-list-without-daemon))
      ...
      (with-selected-frame frame
        ...
        (unless (dotfairy-real-buffer-p (current-buffer))
          (switch-to-buffer (dotfairy-fallback-buffer)))
      ...
      (unwind-protect
          (if (and (not (null +workspaces-on-switch-project-behavior))
            ...
            (with-current-buffer (dotfairy-fallback-buffer)
              (setq default-directory +workspaces--project-dir)
              (hack-dir-local-variables-non-file-buffer)
              (message "Switched to '%s'" (dotfairy-project-name +workspaces--project-dir)))
            (with-demoted-errors "Workspace error: %s"
              (+workspace-rename (+workspace-current-name) (dotfairy-project-name +workspaces--project-dir)))
            (unless current-prefix-arg
              (funcall +workspaces-switch-project-function +workspaces--project-dir)))
        (run-hooks 'projectile-after-switch-project-hook)
        (setq +workspaces--project-dir nil)))))
(provide 'custom-workspace)
;;; custom-workspace.el ends here
